...
const features = [
  ...
  },
  {
    ...
    description: "Typesafe queries, user authentication, and the full power of PostgreSQL.",
    icon: Icons.billing,
  },
  // {
  //   svgPath:
...
export default async function IndexPage() {
      ...
      >
        <div className="mx-auto flex max-w-[58rem] flex-col items-center space-y-4 text-center">
          ...
          <p className="max-w-[85%] leading-normal text-muted-foreground sm:text-lg sm:leading-7">
            Sign up for a free account and start building your own show within the Simulation
          </p>

          {/* Add a download button which is disabled if not logged in a */}


          <Card className="max-w-[85%]">
  <CardHeader>
    <CardTitle>Get Latest Version</CardTitle>
    {user ? (
      <CardDescription>
        For automated updates make sure to create an account on Itch.io.
      </CardDescription>
    ) : (
      <CardDescription>
        Please log in to access the download link.
      </CardDescription>
    )}
  </CardHeader>
  <CardContent className="grid gap-6">
    {user ? (
      <Link
        href={"https://lucy-puppet.itch.io/the-simulation-showrunner"}
        target="_blank"
        className={cn(
      ...
      <button disabled className={cn(buttonVariants({ variant: "secondary", size: "sm" }))}>
        Download
      </button>
    )}
  </CardContent>
